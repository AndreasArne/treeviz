...
class TestMain(unittest.TestCase):
    ...
    def test_tree_to_dot_missing_type(self):
        """
        Test that error is raised when its wrong type
        """
        node_mock = mock.MagicMock()
        with self.assertRaises(ValueError):
            main.tree_to_dot(node_mock, "trie")



    def test_tree_to_dot_root_is_none(self):
        """
        Test that error is raised when root is None
        """
        with self.assertRaises(ValueError):
            main.tree_to_dot(None, "ll")
    ...
    def test_tree_to_png_ll(self, mock_png, mock_ll, mock_dot):
        ...
        """
        node_mock = mock.MagicMock()
        main.tree_to_png(node_mock, "ll", "ll.dot")
    ...
    def test_tree_to_dot_ll(self, mock_ll, mock_dot):
        ...
        """
        node_mock = mock.MagicMock()
        main.tree_to_dot(node_mock, "ll", "ll.dot")
        mock_ll.assert_called_once_with(node_mock)
        mock_dot.to_dot.assert_called_once_with(mock_ll(), "ll.dot")
    ...
    def test_tree_to_dot_bbt(self, mock_bbt, mock_dot):
        ...
        """
        node_mock = mock.MagicMock()
        main.tree_to_dot(node_mock, "bbt", "bbt.dot")
